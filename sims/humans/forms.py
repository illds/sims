...
class HumanForm(FlaskForm):
    name = StringField('Name', validators=[DataRequired(), Regexp(r'^[A-Za-z]+$', message='Name should contain only letters.')])
    surname = StringField('Surname', validators=[DataRequired(), Regexp(r'^[A-Za-z]+$', message='Surname should contain only letters.')])
